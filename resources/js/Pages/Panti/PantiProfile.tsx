...
export default function ProfilPantiDashboard({ pantiData, needs = [] }: { pantiData?: any, needs?: any[] }) {
  ...
  // States form untuk profil
  const profileForm = useForm({
    nama_yayasan: pantiData?.nama_yayasan || 'Yayasan Kasih Ibu',
    deskripsi: pantiData?.deskripsi || '',
    alamat: pantiData?.alamat || '',
    no_telepon: pantiData?.no_telepon || '',
    email: pantiData?.user?.email || '',
    website: pantiData?.website || '',
    tahun_berdiri: pantiData?.tahun_berdiri || '',
    jumlah_anak: pantiData?.jumlah_anak || 0,
    username: pantiData?.username || pantiData?.user?.name || '',
  });

  const posts = pantiData?.posts || [];
  const pengurus = pantiData?.pengurus || [];
  const audits = pantiData?.laporan_audits || [];
  ...
  const formPengurus = useForm({ nama: '', jabatan: '', image: null as File | null });
  const submitPengurus = (e: React.FormEvent) => {
    e.preventDefault();
    formPengurus.post('/panti/pengurus', {
      preserveScroll: true,
      onSuccess: () => {
  ...
  const formAudit = useForm({ judul: '', file_pdf: null as File | null });
  const submitAudit = (e: React.FormEvent) => {
    e.preventDefault();
    formAudit.post('/panti/audits', {
      preserveScroll: true,
      onSuccess: () => {
  ...
  return (
    <div className="w-full bg-white text-[#124354] font-sans pb-20">
      <Head title={`Dashboard Panti - ${pantiData?.nama_yayasan}`} />
        ...
        <div className="px-5 md:px-8 pb-4">
          ...
          <div className="mt-4">
            <h1 className="text-xl md:text-2xl font-black text-[#124354] flex items-center gap-1.5">
              {pantiData?.nama_yayasan}
              <CheckCircle2 size={18} className="text-blue-500 fill-blue-50" />
            </h1>
            <p className="text-gray-500 text-sm">@{pantiData?.username || pantiData?.user?.name}</p>
          </div>
      ...
      <div className="flex overflow-x-auto no-scrollbar border-y border-gray-200 gap-8 md:gap-28 sticky top-0 z-30 bg-white/95 backdrop-blur-md px-5 justify-start sm:justify-center">
        {profileTabs.map((tab) => (
          <button
            ...
            className={`relative px-2 md:px-5 py-3.5 text-[13px] md:text-[14px] font-bold whitespace-nowrap transition-colors hover:bg-gray-50 ${activeProfileTab === tab.id ? 'text-[#124354]' : 'text-gray-500'
              }`}
          >
            {tab.label}
          ...
          <div className="w-full max-w-4xl mx-auto min-h-screen">
            ...
            <div className="divide-y divide-gray-100">
              {posts.length > 0 ? posts.map((post: any) => (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-1">
                        <div className="flex items-center gap-1.5 flex-wrap pr-2">
                          <span className="font-bold text-sm md:text-base truncate">{pantiData?.nama_yayasan}</span>
                          <CheckCircle2 size={14} className="text-blue-500 fill-blue-50 shrink-0" />
                          <span className="text-gray-500 text-xs md:text-sm truncate">@{pantiData?.user?.name} · {new Date(post.time).toLocaleDateString()}</span>
                        </div>
                ...
                <div className="p-10 text-center text-gray-500">Belum ada postingan.</div>
              )}
            </div>
          </div>
        )}
          ...
          <div className="p-5 md:p-8 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {needs.length > 0 ? needs.map((need: any, idx: number) => {
                const targetVal = need.target || need.jumlah || 0;
                const terkumpulVal = need.terkumpul || 0;
                const sisa = Math.max(targetVal - terkumpulVal, 0);
                const progress = targetVal > 0 ? Math.min((terkumpulVal / targetVal) * 100, 100) : 0;
                  ...
                  <div key={idx} className={`border ${isUrgent ? 'border-red-200' : 'border-gray-200'} rounded-xl p-5 bg-white hover:border-[#407E8C] transition-all flex flex-col justify-between shadow-sm group relative overflow-hidden`}>
                    ...
                    <div>
                      <div className="flex justify-between items-start gap-2 mb-2">
                        <h4 className="font-extrabold text-[15px] text-[#124354] leading-tight pr-6">{need.barang || need.nama_kebutuhan || need.nama}</h4>
                        {/* CRUD Actions */}
                ...
                <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-10 text-gray-500 bg-gray-50 rounded-xl border border-gray-200 border-dashed">
                  Belum ada daftar target kebutuhan barang yang dipublikasikan.
                </div>
              )}
            </div>
          </div>
                ...
                <div className="space-y-4">
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#124354] leading-relaxed">{pantiData?.alamat}</p>
                    </div>
                  </div>
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#124354]">{pantiData?.no_telepon || '-'}</p>
                    </div>
                  </div>
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#124354]">{pantiData?.user?.email}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {audits.length > 0 ? audits.map((audit: any) => (
              <div key={audit.id} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 group">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full md:w-auto">
                  <a href={'/storage/' + audit.file_pdf} target="_blank" rel="noreferrer" className="w-full sm:w-auto text-xs font-bold text-[#124354] border border-gray-200 bg-gray-50 px-5 py-2.5 rounded-lg hover:bg-gray-100 transition-all text-center">
                    Lihat File
                  </a>
      ...
      {isEditProfileModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#124354]/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
            ...
            <form onSubmit={submitProfileUpdate} className="p-8 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Nama Yayasan/Panti</label>
                  <input type="text" value={profileForm.data.nama_yayasan} onChange={e => profileForm.setData('nama_yayasan', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Username Publik (opsional)</label>
                  <input type="text" value={profileForm.data.username} onChange={e => profileForm.setData('username', e.target.value.replace(/\s/g, ''))} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" />
                  {profileForm.errors.username && <p className="text-red-500 text-xs mt-1 ml-1">{profileForm.errors.username}</p>}
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Deskripsi / Bio Singkat</label>
                <textarea rows={4} value={profileForm.data.deskripsi} onChange={e => profileForm.setData('deskripsi', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354] resize-none" />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">No. Telepon / WA</label>
                  <input type="text" value={profileForm.data.no_telepon} onChange={e => profileForm.setData('no_telepon', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Email Resmi</label>
                  <input type="email" value={profileForm.data.email} onChange={e => profileForm.setData('email', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Website / Linktr.ee</label>
                  <input type="text" value={profileForm.data.website} onChange={e => profileForm.setData('website', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Tahun Berdiri</label>
                  <input type="text" value={profileForm.data.tahun_berdiri} onChange={e => profileForm.setData('tahun_berdiri', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354]" />
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1.5 ml-1">Alamat Lengkap</label>
                <textarea rows={2} value={profileForm.data.alamat} onChange={e => profileForm.setData('alamat', e.target.value)} className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#124354] resize-none" required />
              </div>
          ...
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={submitPengurus} className="space-y-4">
              <div className="flex justify-center mb-4">
                <label className="w-20 h-20 bg-gray-100 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:bg-gray-200 cursor-pointer overflow-hidden">
                  {formPengurus.data.image ? (
                    <img src={URL.createObjectURL(formPengurus.data.image)} className="w-full h-full object-cover" />
                  ) : (
                    <Camera size={24} />
                  )}
                  <input type="file" className="hidden" accept="image/*" onChange={e => formPengurus.setData('image', e.target.files?.[0] || null)} />
                </label>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1.5">Nama Lengkap</label>
                <input type="text" value={formPengurus.data.nama} onChange={e => formPengurus.setData('nama', e.target.value)} required className="w-full p-3 text-sm rounded-xl bg-gray-50 border border-gray-200 outline-none" />
              </div>
              <div>
