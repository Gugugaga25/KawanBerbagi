  ...
  return (
    <div className="space-y-8 pb-20 text-sm relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div>
          <h1 className="text-3xl font-extrabold text-[#293681] tracking-tight">Kebutuhan Panti</h1>
          <p className="text-gray-500 mt-1">Pantau dan kelola daftar logistik yang sedang dibutuhkan saat ini.</p>
        </div>
        <button 
          onClick={openAddModal}
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-2xl bg-[#4274D9] text-white text-sm font-semibold hover:bg-[#293681] hover:shadow-lg hover:shadow-[#4274D9]/20 transition-all shrink-0"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {[
          ...
          { label: 'Mendesak', value: statMendesak, icon: AlertCircle },
          ...
          <div key={i} className="bg-white rounded-2xl p-4 md:px-5 md:py-4 border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-gray-200 transition-colors">
            <div className="w-12 h-12 rounded-xl bg-[#4274D9] flex items-center justify-center text-white shrink-0 shadow-sm">
              <stat.icon size={20} />
            </div>
            <div>
              <p className="text-2xl font-black text-[#293681] leading-none">{stat.value}</p>
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-center bg-white p-2.5 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div className="relative w-full lg:max-w-md flex items-center px-3 bg-gray-50 rounded-xl border border-transparent focus-within:bg-white focus-within:border-gray-200 transition-colors">
          ...
          <input 
            ...
            className="w-full py-2.5 pl-3 text-sm bg-transparent outline-none border-0 focus:ring-0 focus:outline-none text-[#293681] placeholder-gray-400"
          />
        </div>
          ...
          {['Semua', 'Aktif', 'Mendesak', 'Terpenuhi'].map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap ${
                filter === tab 
                  ? 'bg-[#4274D9] hover:bg-[#293681] text-white shadow-md shadow-[#4274D9]/20' 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            return (
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  {/* Judul Barang */}
                  <h3 className="text-[17px] font-bold text-[#293681] mb-4 leading-snug line-clamp-2">
                    {item.barang}
                  </h3>
                  ...
                  <div className="mt-auto">
                    <div className="flex justify-between items-baseline mb-2">
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-2xl font-extrabold text-[#293681] leading-none">{item.terkumpul}</span>
                        <span className="text-sm font-medium text-gray-400">/ {item.target} {item.satuan}</span>
                      </div>
                      <span className="text-sm font-bold text-[#4274D9]">{percent}%</span>
                    </div>
        ...
        {filter !== 'Terpenuhi' && (
          <button 
            onClick={openAddModal}
            className="flex flex-col items-center justify-center gap-3 bg-transparent rounded-[1.5rem] p-6 border-2 border-dashed border-gray-200 h-full min-h-[220px] hover:border-[#4274D9] hover:bg-[#4274D9]/5 transition-all group"
          >
            <div className="w-12 h-12 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-gray-400 group-hover:text-[#4274D9] group-hover:scale-110 transition-all shadow-sm">
              <Plus size={20} />
            </div>
            <span className="text-sm font-bold text-[#293681] mt-2">Tambah Kebutuhan</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#293681]/60 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-[#293681]">
                Buat Kebutuhan Baru
              </h3>
              <button 
                onClick={handleCloseModal} 
                className="p-2 text-gray-400 hover:text-[#293681] hover:bg-gray-100 rounded-full transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-[#293681] focus:outline-none focus:ring-2 focus:ring-[#4274D9]/20 focus:border-[#4274D9] transition-all"
                />
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border border-gray-200 bg-gray-50">
                <div>
                  <label className="text-sm font-bold text-[#293681] block">Tandai Mendesak</label>
              ...
              <div className="flex gap-3 pt-4 mt-2 border-t border-gray-100">
                ...
                <button 
                  type="submit"
                  disabled={processing}
                  className="flex-1 py-3 bg-[#4274D9] text-white font-bold rounded-xl hover:bg-[#293681] transition-all shadow-md disabled:opacity-50"
                >
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#293681]/60 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-bold text-[#293681] mb-2">Hapus Kebutuhan?</h3>
